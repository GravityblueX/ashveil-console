...
const formFields = computed(
  () => formMap[props.endpoint] || columns.value.filter((c) => c.key !== 'id')
);
...
const formMap = {
  '/access/users': [
    ...
    { key: 'dept', title: '部门' },
    { key: 'roles', title: '角色' },
    { key: 'status', title: '状态', options: ['active', 'locked', 'disabled'] }
  ],
  '/system/dictionaries': [
    { key: 'type', title: '字典编码' },
    { key: 'name', title: '字典名称' },
  ...
  '/audit/logs': [
    { key: 'actor', title: '操作者' },
    { key: 'action', title: '动作', type: 'textarea' },
    { key: 'channel', title: '渠道', options: ['console', 'api', 'scheduler', 'webhook'] },
    { key: 'level', title: '级别', options: ['info', 'warning', 'critical'] }
  ],
  '/jobs': [
    { key: 'name', title: '任务名称' },
    { key: 'cron', title: 'Cron' },
    { key: 'status', title: '状态', options: ['running', 'paused'] }
  ]
};
...
function inferColumns(data) {
  return Object.keys(data[0] || {}).map((key) => ({ key, title: key }));
}
...
function handleAction({ action, row }) {
  modalMode.value = action === '编辑' ? 'edit' : 'view';
  currentRow.value = row;
  modalOpen.value = true;
}
function saveDraft(payload) {
  draftMessage.value = `${modalMode.value === 'create' ? '新增' : '编辑'}草稿已暂存：${Object.values(payload).filter(Boolean).slice(0, 2).join(' / ') || '未填写内容'}`;
...
async function load() {
  ...
  error.value = '';
  try {
    rows.value = await api(props.endpoint);
  } catch (e) {
...
watch(
  () => props.endpoint,
  () => {
    Object.assign(filters, { keyword: '', status: '', channel: '' });
    draftMessage.value = '';
    load();
  }
);
