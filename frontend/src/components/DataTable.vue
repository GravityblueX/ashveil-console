      ...
      <tbody>
        <tr v-for="(row, index) in rows" :key="row.id ?? index">
          <td v-for="column in normalizedColumns" :key="column.key">
            <span v-if="column.type === 'tag'" class="tag" :class="tagClass(row[column.key])">{{
              displayValue(row[column.key])
            }}</span>
            <span v-else-if="column.type === 'list'" class="inline-list">{{
              displayValue(row[column.key])
            }}</span>
        ...
        <tr v-if="!rows.length">
          <td :colspan="normalizedColumns.length + (actions.length ? 1 : 0)" class="empty-cell">
            暂无数据
          </td>
        </tr>
      </tbody>
